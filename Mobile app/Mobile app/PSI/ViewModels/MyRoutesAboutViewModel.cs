...
namespace PSI.ViewModels
{
    ...
    class MyRoutesAboutViewModel : BaseViewModel
    {
        ...
        {
            await Shell.Current.GoToAsync($"{nameof(MyRouteAboutPage)}?{nameof(RouteDetailViewModel.RouteId)}={routeId}");
        }
        ...
        {
            await Shell.Current.GoToAsync($"{nameof(MyRouteWaypointPage)}?{nameof(WaypointDetailViewModel.WaypointId)}={waypoint.Id}");
        }

        private async void OpenMapView()
        {
            await Shell.Current.GoToAsync($"{nameof(Views.Map)}?{nameof(MapViewModel.SelectedRouteId)}={routeId}");
        }
    }
}
